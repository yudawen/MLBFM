...
from lib.models.dance_lib.utils.snake import snake_config
...
def get_box(box):
    ...
    # hh=y_max-y_min
    # ww=x_max-x_min
    # x_min=x_min-1#int(0.1*ww)
    # x_max=x_max+1#int(0.1*ww)
    #
    # y_min=y_min-1#int(0.1*hh)
    # y_max=y_max+1#int(0.1*hh)
    # margin= 32
    # scale = 4
    # start=margin//scale
    # end=start+128
    #
    # x_min = torch.clip(x_min - 0.25, start,end)
    # y_min = torch.clip(y_min - 0.25, start,end)
    # x_max = torch.clip(x_max + 0.25, start,end)
    # y_max = torch.clip(y_max + 0.25, start,end)
