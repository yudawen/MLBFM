import torch
import numpy as np
from lib.models.dance_lib.utils.snake import snake_decode, snake_config
from lib.models.dance_lib.csrc.extreme_utils import _ext as extreme_utils
...
def img_poly_to_can_poly(img_poly):
    ...
    return can_poly
def reorder_polygon_top_left_clockwise_batch(poly):
    """
    poly: (B, N, P, 2)
    功能：
      1. 每个多边形起点为左上点 (x最小 + y最小)
      2. 保证顺时针方向
    """
    B, N, P, _ = poly.shape
    ...
    # -------------------------------
    # x + y 最小点
    lt_idx = torch.argmin(poly[...,0] + poly[...,1], dim=2)  # (B,N)

    # roll 到左上点
    idx = torch.arange(P, device=poly.device)[None, None, :].expand(B, N, P)
    idx = (idx - lt_idx[..., None]) % P  # (B,N,P)
    poly = poly.gather(2, idx[..., None].expand(B, N, P, 2))
    ...
    x = poly[...,0]
    y = poly[...,1]
    area2 = torch.sum(x * torch.roll(y, -1, dims=2) - y * torch.roll(x, -1, dims=2), dim=2)  # (B,N)
    need_flip = (area2 > 0)[..., None, None]  # CCW → True
    poly_flipped = torch.flip(poly, dims=[2])
    poly = torch.where(need_flip, poly_flipped, poly)
