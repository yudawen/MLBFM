...
def decode_ct_hm(ct_hm, wh, reg=None, K=100,cat_spec_wh=True):
    batch, cat, height, width = ct_hm.size()
    ct_hm = nms(ct_hm)

    scores, inds, clses, ys, xs = topk(ct_hm, K=K)
    whs = transpose_and_gather_feat(wh, inds)
    # height=transpose_and_gather_feat(hmap, inds)
    # whs = whs.view(batch, K, 2)
    # wh=whs[:,:,:2]

    if cat_spec_wh:
      wh = whs.view(batch, K, cat, 2)
      clses_ind = clses.view(batch, K, 1, 1).expand(batch, K, 1, 2).long()
      wh = wh.gather(2, clses_ind).view(batch, K, 2)
    else:
      wh = whs.view(batch, K, 2)
    ...
    # height = height.view(batch, K, 1).float()
