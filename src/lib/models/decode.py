...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=64):
    batch, cat, height, width = heat.size()
    ...
    if reg is not None:
      ...
      ys = ys.view(batch, K, 1) + 0.5
    # if hmap is not None:
    #     height = _transpose_and_gather_feat(hmap, inds)
    #     height = height.view(batch, K, 1)

    whs = _transpose_and_gather_feat(wh, inds)
