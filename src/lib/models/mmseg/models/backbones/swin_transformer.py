...
class LRCS(nn.Module):
    def __init__(self, in_channels,num_heads=1,feature_map_size=1):
    ...
    def forward(self, x,label=None):
        ...
        #
        # tf = x[:,:,:half_h,:].clone()
        # tf_view = tf.permute(0, 3, 2, 1).reshape(b * w, half_h, c).contiguous()  # b,w,half_h,c
        # bf = x[:,:,half_h:,:].clone()
        # bf_view = bf.permute(0, 3, 2, 1).reshape(b * w, half_h, c).contiguous()  # b,w,half_h,c
        # x1 = self.msa_v1(tf_view, bf_view).reshape(b, w, half_h, c).permute(0, 3, 2, 1).contiguous() + tf
        # x2 = self.msa_v2(bf_view, tf_view).reshape(b, w, half_h, c).permute(0, 3, 2, 1).contiguous() + bf
        #
        # x=torch.cat((x1,x2),dim=2)
        # x = self.conv_v(x)
        #
        # lf = x[:,:,:,:half_w].clone()
        # lf_view = lf.permute(0, 2, 3, 1).reshape(b * h, half_w, c).contiguous()  # b,w,half_h,c
        # rf = x[:,:,:,half_w:].clone()
        # rf_view = rf.permute(0, 2, 3, 1).reshape(b * h, half_w, c).contiguous()  # b,w,half_h,c
        # x1 = self.msa_h1(lf_view, rf_view).reshape(b, h, half_w, c).permute(0, 3, 1, 2).contiguous() + lf
        # x2 = self.msa_h2(rf_view, lf_view).reshape(b, h, half_w, c).permute(0, 3, 1, 2).contiguous() + rf
        #
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        # self.attn= LRCS(dim,1,feature_map_size)
