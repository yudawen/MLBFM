...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='BF',
                             help='coco | kitti | coco_hp | pascal | pylon | dior |building|NWPU|tzb|ROSD')
    self.parser.add_argument('--exp_id', default='BUFF')
    ...
    self.parser.add_argument('--demo', default='',
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--load_model', default=r""
,
                             help='path to pretrained model,e.g.\model_last.pth')
    self.parser.add_argument('--resume', default=0,
    ...
    self.parser.add_argument('--nms', default=True,
                             help='run nms in testing.')
    self.parser.add_argument('--K', type=int, default=64,
                             help='max number of output objects.') 
    self.parser.add_argument('--not_prefetch_test', default=True,
                             help='not use parallal data pre-processing.')
    self.parser.add_argument('--fix_res', default=True,
  ...
  def parse(self, args=''):
    ...
    if opt.head_conv == -1: # init default head_conv
      opt.head_conv = 64 if 'dla' in opt.arch else 256
    opt.pad = 127 if 'hourglass' in opt.arch else 31
    opt.num_stacks = 2 if opt.arch == 'hourglass' else 1
    print(opt.trainval,"*"*20)
    if opt.trainval:
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [800, 800], 'num_classes': 10,
